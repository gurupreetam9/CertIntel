...
# Use environment variable for API key, with a fallback. The hardcoded key is invalid.
# You should set a valid COHERE_API_KEY in your environment for the AI fallback to work.
COHERE_API_KEY = os.getenv("COHERE_API_KEY", "jrAUYREK77bel5TGil5uyrzogksRcSxP78v97egn")
co = cohere.Client(COHERE_API_KEY) if COHERE_API_KEY else None

if not co: 
    logging.warning("COHERE_API_KEY not found or is invalid. LLM features will be limited.")

TESSERACT_PATH = shutil.which("tesseract")
if not TESSERACT_PATH: 
    logging.critical("Tesseract OCR not found in PATH. Pytesseract will fail.")
else: 
    logging.info(f"Tesseract OCR found at: {TESSERACT_PATH}")
...
def query_llm_for_course_from_text(text_content: str) -> Optional[str]:
    ...
    try:
        ...
        logging.error(f"Cohere course extraction error: {e}")
        return None
...
def infer_course_text_from_image_object(pil_image_obj: Image.Image) -> Tuple[List[str], str]:
    ...
    if model and TESSERACT_PATH:
        try:
            img_np = np.array(pil_image_obj)
            # Handle different image modes correctly before color conversion
                    ...
                    if names[int(box.cls[0].item())].lower() in ["certificatecourse", "course", "title"]:
                        ...
                        if text: 
                            ...
                            if courses_from_region: 
                                return list(set(courses_from_region)), "SUCCESS_YOLO_OCR"
        except Exception as yolo_err: 
            logging.error(f"YOLO inference error: {yolo_err}", exc_info=True)
            status = "FAILURE_YOLO_ERROR"
        ...
        try:
            ...
            if text and len(text) >= 5:
                ...
                if llm_course: 
                    ...
                    if courses_from_llm: 
                        return list(set(courses_from_llm)), "SUCCESS_LLM_EXTRACTION_FROM_FULL_OCR"
                status = "FAILURE_LLM_OUTPUT_FILTERED_EMPTY" if llm_course else "FAILURE_LLM_NO_COURSE_IN_TEXT"
            else: 
                status = "FAILURE_FULL_IMAGE_OCR_NO_TEXT"
        except Exception as ocr_err: 
            status = f"FAILURE_FULL_IMAGE_OCR_ERROR: {str(ocr_err).splitlines()[0]}"
    elif not TESSERACT_PATH: 
        status = "FAILURE_TESSERACT_NOT_FOUND"
...
def query_llm_for_detailed_suggestions(known_course_names_list_cleaned: List[str]):
    ...
    try:
        ...
        logging.error(f"Cohere suggestions error: {e}")
...
def parse_llm_detailed_suggestions_response(llm_response_text: str) -> List[Dict[str, Union[str, None, List[Dict[str, str]]]]]:
    ...
    for block in blocks:
        ...
        if not orig_match: 
            logging.warning(f"LLM Parser: No 'Original Input Course' in block: {block[:100]}...")
            continue
        ...
        if sug_blob_match:
            ...
            if sug_blob.lower() != "no specific suggestions available for this course.":
                ...
                for i, part in enumerate(ind_sugs):
                    ...
                    if name_m and desc_m and url_m: 
                        suggestions.append({"name": name_m.group(1).strip(), "description": desc_m.group(1).strip(), "url": url_m.group(1).strip()})
                    else: 
                        logging.warning(f"LLM Parser: Malformed suggestion for '{orig_course}': {full_sug_block[:100]}...")
...
def get_course_recommendations(
    ...
    for name in raw_names:
        ...
        if cleaned and cleaned not in cleaned_map: 
            cleaned_for_llm.append(cleaned)
            cleaned_map[cleaned] = name
        elif not cleaned and name: 
            logging.warning(f"Suggestions: Raw name '{name}' empty after cleaning.")
    
    if not cleaned_for_llm: 
        return {"user_processed_data": [], "llm_error_summary": "No valid courses for suggestions (after cleaning)."}
